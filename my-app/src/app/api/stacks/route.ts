...
export async function GET() {
  ...
  const stacksFontEnd = myStacks.filter((stack) => stack.type === "front-end");
  const stacksBackEnd = myStacks.filter((stack) => stack.type === "back-end");
  const stacksTools = myStacks.filter((stack) => stack.type === "tools");

  return NextResponse.json({ stacksFontEnd, stacksBackEnd, stacksTools });
}
